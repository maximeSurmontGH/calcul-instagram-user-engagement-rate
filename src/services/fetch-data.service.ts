...
export const fetchData = async (): Promise<IInstagramApiRootPageResponse[]> => {
  const accountDatasPromises = ACCOUNT_NAMES.map(async (accountName) => {
    ...
    let endCursor =
      accountData.graphql.user.edge_owner_to_timeline_media.page_info.end_cursor
    do {
      ...
      const accountDataToAdd: IInstagramApiPageResponse = pageAxiosResponse.data
  ...
  const accountDatas = await Promise.all(accountDatasPromises)
  return accountDatas
}
